...
@comment = @hall.comments.find(params[:id])
  ...
  def create
     @hall = Hall.find(params[:hall_id])  
     @comment = @hall.comments.create(comment_params) 
     @comment.user_id = current_user.id 

    respond_to do |format|
      if @comment.save
        format.html {  redirect_to hall_path(@hall), notice: 'Comment was successfully created.' }
  ...
  private
    ...
    def comment_params
      params.require(:comment).permit(:content, :hall_id, :user_id)
    end
end
