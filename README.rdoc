...
<div class="field">
 ...
 Za delovanje Google logina moramo najprej namestiti ustvariti aplikacijo v Konzoli razvijalcev google. ((((((((UPDATE!))))))) Potem sva dodala gem v Gemfile z vrstico <tt>gem "omniauth-google-oauth2", "~> 0.2.1"</tt> in ustvarila omniauth.rb v mapi config/initializers.
 V omniauth.rb sva dodala naslednje vrstice: 
 <tt>OmniAuth.config.logger = Rails.logger
...
'my Google client id' in 'my Google client secret' moramo zamenjati s številkami ki jih dobimo v https://console.developers.google.com .
Ustvariti sva morala tudi model za Google uporabnike s komando <tt>rails g model google_user provider uid name oauth_token oauth_expires_at:datetime</tt>.
Za preizkus Google logina sva ustvarila še controller in view z <tt>rails g controller home show</tt> in zraven dodala še <tt>rails g controller Sessions create destroy</tt> za uporabo sessiona s katerim se uporabnik vpiše in izpiše.
Potem sva sledila korakom iz spletne strani https://richonrails.com/articles/google-authentication-in-ruby-on-rails/ .